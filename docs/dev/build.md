...
# Or:
./scripts/build.sh -p=arm64 -b=ubuntu:24.04 -n=humblebeeai -r=rest.fastapi-template -v=1.0.0 -s=-arm64 -d=./Dockerfile -t=. -u -c
```
...
docker build \
    --progress plain \
    -t humblebeeai/rest.fastapi-template:latest \
    .

# Push image to Docker Registry:
docker push [IMG_FULLNAME]
# For example:
docker push humblebeeai/rest.fastapi-template:latest
```
...
docker buildx build \
    --progress plain \
    --platform linux/amd64,linux/arm64 \
    --cache-from=type=registry,ref=humblebeeai/rest.fastapi-template:cache-latest \
    --cache-to=type=registry,ref=humblebeeai/rest.fastapi-template:cache-latest,mode=max \
    -t humblebeeai/rest.fastapi-template:latest \
    --push \
    .
